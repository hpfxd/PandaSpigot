...
if [ ! -d "$classdir" ]; then
    ...
    cd "$classdir"
    set +e
    jar xf "$decompiledir/$minecraftversion-mapped.jar" net/minecraft/server assets
    if [ "$?" != "0" ]; then
        cd "$basedir"
...
if [ ! -d "$pandadecompiledir/net/minecraft/server" ]; then
    ...
    cd "$basedir"
    set +e
    java -jar "$basedir/bin/fernflower.jar" -dgs=1 -hdc=0 -asc=1 -udv=0 -aoa=1 "$classdir" "$pandadecompiledir"
    if [ "$?" != "0" ]; then
